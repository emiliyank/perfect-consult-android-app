...
class Database(context: Context) {
    ...
    fun getLastSyncedCallTimestamp(listener: DataListener<Long>){
        SettingRetrieveTask(Settings.OWNER_PHONE, database, object: DataListener<Settings> {
            override fun onData(settings: Settings) {
                listener.onData(settings.value.toLong())
            }
        } ).execute()
    }
    ...
    private class SettingRetrieveTask(val setting: String, val database: AppDatabase, val listener: DataListener<Settings>) : AsyncTask<Void, Void, Settings>(){
